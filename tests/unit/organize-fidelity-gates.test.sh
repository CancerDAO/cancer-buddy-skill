...
#   - validate_structured_outputs.py lab source-shape integrity
...
: > "$d/raw/clinical/张测试-OncoFusion报告-123456789.pdf"
cat > "$d/INDEX.md" <<'EOF'
# patient_code: PT-TEST01
| f074 | NGS | raw/clinical/张测试-OncoFusion报告-123456789.pdf | text |
EOF
...
# ---------------------------------------------------------------------------
# 4. lab source shape: missing per-result source fields → block
# ---------------------------------------------------------------------------
mkdir -p "$tmp/num"
cat > "$tmp/num/labs.json" <<'EOF'
{ "patient_code":"PT-TEST03","schema_version":"2","panels":[
