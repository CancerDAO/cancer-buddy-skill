...
| 深夜睡不着、焦虑、甚至不想活了 | PHQ-9 / GAD-7 / C-SSRS（官方验证版，可选不强制）；危机时先陪你评估当下安全，再给已核实的当地求助入口 |
...
case-precedent  找 PubMed/EPMC 上和你相似的真实病例（case report），看别人怎么治、后来怎么样——线索非预后，强制偏倚披露
visit-prep      一页就诊准备包：医生 30 秒速览 + 该问医生什么 + 带什么 + 复诊"上次→这次"变化
```

你不需要按顺序使用。系统会先理解你的身份（患者 / 照护者 / 家属），再根据情境引导下一步。

搭子还会**按你病历的语言自动本地化输出**：英文病历就出英文档案，法语病历就出法语，无需手动设置（检测到的语言会记进你的档案，整个旅程保持一致）。但药名、基因、变异、TNM、数值单位这些临床术语一律保持原文逐字——误译就是医疗风险，本地化只动章节标题、字段标签和叙述文案。

organize 的运行机制是**运行时中立**的：行为契约与执行机制分层，可由 Claude Code、codex 或其它 headless agent 驱动同一份契约，逻辑与产物结构不变。
...
- 把信息整理清楚（病历、时间线、指标变化）——每次补充材料后自动更新「病情简要总结」，用趋势图呈现肿瘤标志物/化验的走势、与治疗方案的对应，并标出「自上次总结的变化」
- 帮你准备问题（提升每次就诊效率）
- 在情绪压力下提供支撑
...
`--all` 会把 13 个子技能（搭子主入口 + 11 个陪伴模块 + 1 个联网底层 web-access）一次性全部装好，不用手动勾选。装完重启 Claude Code，对它说 `抗癌搭子` 或 `帮我分析病情` 就能用。
...
抗癌搭子    ❯ 你已经扛得很久了。先做个很快的自评：
              ...
              如果出现"想消失、想结束"这种念头，
              可以拨 12356（全国心理援助热线）；
              情况紧急时立刻打 120 或去最近的急诊。
              就算暂时不想打电话，我也会一直在这儿陪你。
```
...
├── INSTALL.md                         # 详细安装说明
└── skills/
    ├── cancer-buddy/                  # 总入口（路由到下面 11 个 companion）
    ...
    ├── cancer-buddy-case-precedent/   # 找相似真实病例（case report），线索非预后
...
整理好的档案目录里还会自带一份 `AGENTS.md` 索引：兼容的 agent 工具（如 pi、Claude Code）在该目录下开新会话时，能自动认出"这是谁的档案"、按问题定向读取对应文件——不用每次重新指路，也不必先手动唤起搭子，下次接着问就行。
