...
3. **Read de-identified sources** (read-only): `patient_summary.json`, source/faithfulness flags, `molecular.json`, `treatment_lines.json`, `labs.json`, `timeline.json`, `missing_items.json`. Never read `raw/`.
4. **Map Block 1 医生速览** — direct source mapping; null → `val_pending`. Do not calculate stage, ECOG, response, progression or treatment line.
...
- [scripts/validate_visit_prep_html.py](scripts/validate_visit_prep_html.py) — deterministic form/shape PII validator. Contextual minimization of age and other quasi-identifiers remains a producer/authorization review.
