...
> **来自 Cancer Buddy 的临床检索必读**：涉及患者临床/法律/机构/试验信息时，本 skill 通用的「登录后操作 / 提交表单 / 上传文件」能力**一律不启用**——按下文「Cancer Buddy 临床检索覆盖规则」走**严格只读**（禁上传患者文件、禁提交表单、禁状态变更；找不到官方一手来源即 fail closed，不以模型记忆/搜索摘要补写）。
