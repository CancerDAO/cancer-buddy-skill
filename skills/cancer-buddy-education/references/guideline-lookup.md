...
- **(b) 指南级断言**（"NCCN/CSCO/ESMO 指南建议 / 标准治疗是什么 / 一线二线是什么方案 / 我这类一般用什么药 / 最新获批"）＝**版本敏感的外部目录事实**，落 `../../../references/safety-guardrails.md` no-silent-snapshot 红线 → **走本文件：实时联网检索，禁 LLM 凭记忆合成**。

边界模糊时**倾向 (b)**：宁可查也不凭记忆答指南。
...
| P2 | **NCCN** | 版权 / 登录墙 | **只"指向 + 引其 category 级别"，不复制表格全文**；能用 PDQ/CSCO 覆盖就不碰 NCCN 原文 |
...
网络不可达或某源无法确认 → 该条标 `需现场核实`，**不静默降级到静态模块或模型记忆**。
...
- **G-NO-SYNTH**：指南级断言只能来自真实抓取、可逐字回溯的源；LLM 记忆不得挂角标。
