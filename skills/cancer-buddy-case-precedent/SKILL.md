...
**(c) 只有癌种也能先给一个真实例子（别让 payoff 全押在 onboarding 后）**：若用户已给出癌种（哪怕只有"直肠癌"），主动提议**现在就先拉一个真实的、带可点 PMID 的示例个案**给 Ta 看——"要不要我现在就按'直肠癌'先找一个真实的例子给你看看别人的经历长什么样？先粗匹配，我会标清楚哪里像、哪里还不确定。" 这样 Ta 这一轮就能看到**真实个案**（非示意卡），而不是只拿到问卷——**提议即附上那一例**（"先粗匹配的一例，哪里像 / 哪里还不确定我都标了"），把"再问一轮"的往返降为零，别让过度谨慎变成延迟交付。患者版用**人话表达相似度**（如"同癌种 + 同为治疗中途换线"），不上 6 维技术评分。仍旧：**这是他人经历、不是给你的治疗建议**，逐例结局如实（含 `deceased`/`progression`），不编 PMID。
