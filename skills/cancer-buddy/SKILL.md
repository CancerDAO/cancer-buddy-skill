...
- 开口用**不泄露敏感细节**的锚点确认，例如 `上次我们聊的是肺部的情况，对吗？`——只提大方向（部位/大类），不点出确诊病名、分期、机构或具体文件。
- **确认为"对"之后，才**读取并呈现完整诊断、机构、来源引用与文件路径。用户说"不是"或含糊 → 停下，回到身份询问，别继续读这个档案。
...
- **可控可删**：`这些都是给你存的，想删想改随时说。`——让患者知道档案是 Ta 的、随时可控可删（呼应 `authorization-and-consent.md` 的可撤销/可删除边界）。
