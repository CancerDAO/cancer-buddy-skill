...
- **(b) 指南级**（"指南/NCCN/CSCO/ESMO 怎么建议 / 标准治疗是什么 / 一线二线什么方案 / 我这类一般用什么药 / 最新获批"）＝**版本敏感的外部目录事实** → **必须走 `cancer-buddy-education` 的 `guideline-lookup.md` 子路径 live-first 实时联网检索**（`../../references/safety-guardrails.md` no-silent-snapshot 已点名"guideline versions"）：联网拿到 → **联网锚编号引用**（见「来源引用」节）；联网**不可达** → 按该文件「优雅降级」给**显式标注（未经实时核实·基于模型知识）、不挂角标**的兜底，**绝不静默把记忆当已核实来源**。呈现仍是"对你这类情况一般…"的条件图 + "要不要换由主诊医生定"收口，**不是个人换线判决**。边界模糊时倾向 (b)。
...
- **脚注列表必须出现在面向用户的回答本身里**（即便另有 `.md` 落盘接地记录）：只给内联 `[n]` 却不给可解析的 `[n] → 源` 列表，患者查不到源 = 无效引用。内联出现的最大 `[n]` = 列表长度，两边编号一一对应。
