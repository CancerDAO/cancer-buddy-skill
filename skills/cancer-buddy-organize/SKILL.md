...
For short text that the health-vault owner has already reviewed and explicitly approved (for example, a family-submitted temperature/symptom diary), call this same mode with `owner_reviewed: true` and follow [`references/owner-reviewed-text.md`](references/owner-reviewed-text.md). This is a fast branch of段C, not a fourth organize mode: it skips the duplicate diff-card gate, OCR, full synthesis, readiness, reconciliation, and case-summary generation while retaining local PII masking, `patient_curated` provenance, append-only writes, and the existing 14-domain schema.
...
- [owner-reviewed-text.md](references/owner-reviewed-text.md) — 段C fast branch for owner-approved short text: no duplicate confirmation, no OCR/full synthesis, at most one masked-text semantic call, append-only diary/observation writes
