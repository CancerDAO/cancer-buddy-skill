...
18. **Safe export (on demand, when the user wants a shareable copy)** — see the **Safe export** section below. Produce the share copy with `scripts/export_share.py`, which refuses to run unless the acceptance gate passes first, so a copy carrying raw originals or residual PII never ships.
...
- **refuses to run unless `python3 scripts/validate_structured_outputs.py <patient_dir>` passes first** — the acceptance gate (schema + anchors + `gate_pii_rescan` incl. the delivered surfaces INDEX.md / source_inventory.json / update_log.json / dotfiles / 病情简要总结.html + `gate_numeric_integrity` + `source_inventory.json` completeness + case-summary HTML shape) must be green, so no PII / path leak / unfaithful value ships in a shared copy. If the gate fails, the export aborts with the gate's errors and writes nothing.
