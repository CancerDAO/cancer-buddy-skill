...
- `01_身份与基础信息/`…`14_患者自管补充/` (14 clinical-domain raw file buckets, scheme_version 3 — see `references/bucket-taxonomy.md`; filenames follow `<YYYY-MM-DD>_<doc_type>_<hospital>.<ext>` — 4-level hospital fallback). Each file is co-located with its **text-masked MD sidecar** at `<bucket>/<canonical>.md` (the downstream-only read source — no plaintext PII).
...
- Whatever the binding, it produces the **same canonical output set** the contract defines (14 clinical buckets + co-located 文本脱敏 MD, `raw/` verbatim originals, `source_inventory.json`, `profile.json`, `timeline.*`, `case_text.md`, `readiness.json`, `review_flags.md`, the 6 structured JSONs, `missing_items.json`, `update_log.json`, bucket-relative anchors) and honors the same invariants (sidecar text is LLM-generated; sidecar is the only downstream plaintext boundary and the only desensitization layer; uploaded originals in `raw/` are kept verbatim; unconfirmed → no formal write / no irreversible delete; clinical entities verbatim; logic/schema/产物结构 unchanged).
