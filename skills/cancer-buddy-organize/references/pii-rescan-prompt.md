...
- **合成下游正文面**（整文件扫）：`case_text.md`、`timeline.md`、`profile.json`、`patient_summary.json`、`review_summary.md`、`review_flags.md`。它们由 sidecar 合成，下游/患者向读它们；shape 兜底**不扫**这些（既非桶内 sidecar 也不在已交付面清单），所以"按含义才认得出"的 PII（出生地/籍贯/职业/民族…）只有本层能拦——这正是 sidecar masker 漏过、case_text 泄漏的根因面。
