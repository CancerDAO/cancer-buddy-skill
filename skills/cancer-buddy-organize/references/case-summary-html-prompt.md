...
## 【强制规则1】医学优先级指标筛选（腹膜转移专用）

生成 `trend_charts[]` 列表时，严禁手工挑选或硬编码，必须严格按以下分级执行：

- **一级必选（三联肿瘤标志物）**：只要数据存在，`CA125`、`CEA`、`CA19-9` 必须全部进入趋势图。
- **二级必选（安全底线）**：只要数据存在，`HGB`、`ALB`、`eGFR` 必须全部进入趋势图。
- **强制黑名单（丢弃）**：`AFP`、`LPA`、`CYFRA21-1` 严禁进入趋势图（结直肠癌无临床价值）。
- **仅作文字描述**：`Mg`、`LDH`、`LYM%` 仅在文字段落提及，不绘图。

**规则意图**：腹膜转移结肠癌的三联肿瘤标志物（CA125、CEA、CA19-9）必须全部可视化，以验证免疫化疗疗效；血象和营养/肾功则需监测安全性。无例外。

---

## 【强制规则2】术语自检（防"胰腺/腹膜"混淆）

输出 HTML 前，必须全文检索即将输出的内容：

- 所有诊断描述必须使用 **"腹膜转移"**。
- 若检测到 **"胰腺转移"、"pancreatic"** 等词汇，立即全部替换为"腹膜转移"，否则视为严重医疗事故。
- 检查 `case_summary_narrative` 和所有诊断字段中是否有误用术语。

---

## 【强制规则3】HTML 结构自检（防冗余div/标签错乱）

在输出 HTML 末尾（`</html>`之前），必须执行心理检查：

- 统计 `<div>` 开标签数量是否等于 `</div>` 闭标签数量。
- 统计 `<table>`、`<tr>` 等所有成对标签是否完全匹配。
- 若不等，必须重新整理嵌套结构，补全缺失标签，直至所有成对标签数量相等才能输出。

---

## 【强制规则4】隐私绝对脱敏（红线）

- `basic_info.name` 无论原始值是什么，在 HTML 任何位置（标题、正文、图注）只能显示为 **"患者"** 或 **"Pt. M2"**。
- 若输出文本中出现 **Elizabeth、Martinez** 等真实姓名，立即删除并替换为 **"患者"** 。
- 绝不输出出生日期、出生地、籍贯、职业、工作单位等可识别信息。
