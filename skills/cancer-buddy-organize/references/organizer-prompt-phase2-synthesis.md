...
e.g. for `locale = en` (and every other non-`zh` locale) the domains are `01_identity_basics / 02_history_family / 03_clinical_notes / 04_diagnosis_staging / 05_imaging / 06_molecular_omics / 07_labs / 08_treatment / 09_procedures / 10_followup_monitoring / 11_consult_referral / 12_psychosocial_support / 13_admin_financial / 14_patient_supplement`. The **typed subdirs are also pinned** — use the authoritative `zh ↔ en` map in [`bucket-taxonomy.md`](bucket-taxonomy.md) §1.1a (e.g. `04_diagnosis_staging/pathology`, `06_molecular_omics/ngs`, `07_labs/tumor_markers`); they are `zh` or `en` pinned (§1.1a), **never** runtime-translated per the locale's rendering. `high_confidence` / `uncertain` / `conversation_notes` stay ASCII as-is. Whatever the locale, **build every `bucket_path` / `md_dest` / anchor with the same pinned slug consistently** so the on-disk path and the anchor agree.
...
- NEVER pixel-redact or delete an uploaded original in `raw/` — originals are kept verbatim; the only CONTENT-level desensitization of the archived data is the text masking in the `.md` sidecar (the raw/ on-disk filename is separately de-identified by Phase 1) (Phase 1) re-scanned by `pii_rescan.py`.
