...
| LLM 输入源 | sidecar 正文和 PII locator 由 LLM 输出,纯 OCR/parser 不是临床正文或 PII 判断来源 | worker 内 `Read` 读取图片/渲染页/文件 payload,由模型写脱敏 MD 和 `.pii_regions.json` |
...
- sidecar 正文和 PII locator 必须由 Claude/driver LLM 读取 adapted input 后写出。
...
- LLM 可生成 MD/JSON/HTML 前置数据和 PII locators;确定性 HTML 渲染、PII rescan、redaction coverage gate 由脚本执行,source redaction QA 由 LLM QA report gate。
