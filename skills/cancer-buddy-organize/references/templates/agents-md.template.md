...
<!--
  本文件由 cancer-buddy-organize 自动生成（填充模板 references/templates/agents-md.template.md，与 case-summary.template.html 同处 organize 的产物模板目录；organize 是唯一填充者）。生成出的 AGENTS.md（患者目录里）才是被 cancer-buddy 家族 + vmtb 共用的对象。
