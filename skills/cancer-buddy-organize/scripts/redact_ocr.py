#!/usr/bin/env python3
# vendored-from: cancer-buddy-organize-local-skill/skills/cancer-buddy-organize-local/scripts/redact_ocr.py (verbatim; sync upstream changes here)
"""
...
_SEP = r"\s*[:：\s]\s*"  # colon, full-width colon, or whitespace as separator

_PII_LABEL_PATTERNS = [
    (re.compile(r"(姓?\s*名|患\s*者信息|患\s*者|病\s*人)" + _SEP), "patient_name"),
    (re.compile(r"(身份证号?码?|证件号码?|ID)" + _SEP), "id_number"),
    (re.compile(r"(电\s*话|联系电话|手\s*机|联系方式|Tel|TEL)" + _SEP), "phone"),
    (re.compile(r"(地\s*址|住\s*址|通讯地址|联系地址)" + _SEP), "address"),
    (re.compile(r"(住院号|住院病历号|病历号|门诊号|就诊号|就诊卡号|病案号|报告单号|ID号)" + _SEP), "admission_id"),
    (re.compile(r"(床\s*号|病\s*床|床\s*位)" + _SEP), "bed_number"),
    (re.compile(r"(检验者|检验医师|检验人员|检测者)" + _SEP), "examiner"),
    (re.compile(r"(审核者|审核医师|审核人员|审核人)" + _SEP), "reviewer"),
    (re.compile(r"(报告医师|报告者|报告人|报告医生)" + _SEP), "reporter"),
    (re.compile(r"(核对者|核对人|复核者|复核人)" + _SEP), "verifier"),
    (re.compile(r"(送检医[师生]|开单医[师生]|临床医[师生]|主治医[师生]|申请医[师生])" + _SEP), "ordering_doctor"),
]
...
# ---------------------------------------------------------------------------
# OCR instance cache — one PaddleOCR per thread (PaddleOCR isn't thread-safe)
# ---------------------------------------------------------------------------

import threading as _threading

_ocr_tls = _threading.local()
_ocr_version = None  # "v3" | "v2" | None — shared across threads once detected


def _build_ocr_instance():
    """Create a fresh PaddleOCR instance; detect version on first global call."""
    global _ocr_version

    # Try v3 first (prefer if we already know v3 works, or on the very first attempt)
    if _ocr_version in (None, "v3"):
        try:
            import os
...
def _get_ocr_instance():
    """Return this thread's cached PaddleOCR instance, creating on first call."""
    inst = getattr(_ocr_tls, "instance", None)
    ver = getattr(_ocr_tls, "version", None)
    if inst is not None:
        return inst, ver

    ocr, version = _build_ocr_instance()
    _ocr_tls.instance = ocr
    _ocr_tls.version = version
    return ocr, version
...
def _run_ocr_v3(ocr, image_path: str, confidence_threshold: float):
    """PaddleOCR 3.x API: ocr.predict()"""
...
def _run_ocr_v2(ocr, image_path: str, confidence_threshold: float):
    """PaddleOCR 2.x API: ocr.ocr()"""
...
def run_ocr(image_path: str, confidence_threshold: float = 0.5):
    ...
    Uses cached instance; detects v3/v2 on first call.

    Returns:
        lines: [{"text": str, "bbox": [[x1,y1],[x2,y2],[x3,y3],[x4,y4]], "confidence": float}, ...]
    """
    ocr, version = _get_ocr_instance()
    if version == "v3":
        return _run_ocr_v3(ocr, image_path, confidence_threshold)
    else:
        return _run_ocr_v2(ocr, image_path, confidence_threshold)
...
def redact_image_ocr(
    ...
    # 8) Build result
    ocr_text_full = "\n".join(x["text"] for x in lines_classified)
    ocr_text_safe = "\n".join(
        x["text"] for x in lines_classified if x["classification"] == "keep"
    )
