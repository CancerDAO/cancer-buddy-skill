...
1. 读 `longitudinal_observations.json`，统计用户问的那个 analyte 的记录数
2. ≥2 个可比点 → 文字回答 **+ 自动附图**（路径 B 的命令）
3. <2 个点或不可比 → 文字回答 **+ 说明为什么没图**（`render_chart.py` 的 exit 5 消息可直接转述）

**边界（违反即为越界，见 references/chart-eligibility.md §7）**
- 只画用户问的那一个指标。**严禁**顺带画其他指标
- 泛问（"我的化验单怎么样"）不触发
- 一次对话最多 1 张图
...
| | 例 |
|---|---|
| ✅ 该写 | 「CA19-9 四次测量 · 第 3 次起检测方法变更，前后不可直接比较」「体重 6 个月 5 次记录 · 中间有 4 个月空档」 |
...
- 无数据不画。任何一个绘制点都必须能在源 JSON 里找到（`--longitudinal` / `--labs` 开启反捏造门，exit 3）
- 参考区间只用该次报告自带的 `reference_range`，**禁止套用通用参考值**；解析不出就不画区间带并明说
- `method_or_device` 变化时序列自动断开，不连成一条线
- 超出参考区间用琥珀描边，**不用红色填充**；红色只留给源报告自己标注的危急值
- 不画趋势方向箭头（向下箭头会被读作"好转"）
