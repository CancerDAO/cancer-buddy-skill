...
Pairs with the clinician-facing [vmtb-skill](https://github.com/zwbao/vmtb-skill). Both plugins read and write the same `patients/<id>/` directory, so records organized in cancer-buddy can be read directly by vmtb-skill's full MTB committee.
...
All records and reports live locally under `patients/<patient_id>/`.

`patient_id` format: `<pinyin_initials>_<diagnosis_yyyymm>`, e.g. `WZM_202312`.
